...
def GetGradeAndPattern():
    ...
    ap.add_argument("-intC", "--Year", required=True, help="Year")
    ap.add_argument("-intD", "--Month", required=True, help="Month")
    ap.add_argument("-intE", "--Day", required=True, help="Day")
    ...
    month = args["Month"]
    day = args["Day"]
    return [grade,pat,year,month,day]

#主程序
initLeanCloud()
[grade,pat,year,month,day] = GetGradeAndPattern()
if int(pat) == 0: patString = "午休"
else: patString = "晚修"
print("获取",year,"年",month,"月",day,"日的"+patString+"检查表...")

# 抓取应到实到数据
ClassData = leancloud.Object.extend('ClassData')
gradeQuery = ClassData.query
dateQuery = ClassData.query
gradeQuery.equal_to('grade',int(grade))
dateQuery.equal_to("date",str(year)+'-'+str(month)+'-'+str(day))
query = ClassData.query
query = leancloud.Query.and_(gradeQuery,dateQuery)
query_list = query.find()

# 抓取扣分数据
SituationData = leancloud.Object.extend('SituationData')
situationQuery = SituationData.query
dateQuery1 = SituationData.query
dateQuery1.equal_to("date",str(year)+'-'+str(month)+'-'+str(day))
situationQuery.equal_to('grade',int(grade))
query2 = leancloud.Query.and_(situationQuery,dateQuery1)
